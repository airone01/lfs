...
select opt in "${options[@]}"; do
        case $opt in
        "Start LFS (GUI)")
                CMD="qemu-system-x86_64 -enable-kvm -m ${LFS_RAM}G -smp $LFS_NPROC -drive file=\"$DISK_IMG_LFS\",format=qcow2 -nic user,model=virtio-net-pci,hostfwd=tcp::2222-:22 -vga virtio -usb -device usb-tablet"
                confirm_and_run "$CMD"
                ;;
        "Start LFS (headless)")
